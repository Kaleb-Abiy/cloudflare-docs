...
:::caution[Python requires python image variant]
...
If you attempt to run Python code using the default lean image, you will receive a `PYTHON_NOT_AVAILABLE` error. JavaScript and TypeScript work with both image variants. See [Dockerfile reference](/sandbox/configuration/dockerfile/) for details on image variants.
:::
