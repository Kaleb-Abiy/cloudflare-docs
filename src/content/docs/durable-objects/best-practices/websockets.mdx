...
class WebSocketHibernationServer(DurableObject):
    ...
    async def fetch(self, request):
        ...
        # Calling `accept_websocket()` connects the WebSocket to the Durable Object, allowing the WebSocket to send and receive messages.
        # Unlike `ws.accept()`, `state.accept_websocket(ws)` allows the Durable Object to be hibernated
...
class WebSocketServer(DurableObject):
    ...
    async def fetch(self, request):
        ...
        server.addEventListener("close", on_close)
